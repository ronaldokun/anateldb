...
__all__ = ['optimize_floats', 'optimize_ints', 'optimize_objects', 'df_optimize', 'connect_db', 'update_radcom',
           'update_stel', 'update_mosaico', 'merge_df', 'read_aero', 'merge_aero', 'update_base', 'read_stel',
...
def read_aero(pasta, up_icao=False, up_pmec=False, up_geo=False):
    ...
    cols = ['Frequency', 'Latitude', 'Longitude', 'Service', 'Station']
    df = merge_df(icao, pmec, cols, how='outer')
    return merge_df(df, geo, cols, how='outer')


def merge_aero(df, other):
    """Mescla a base da Anatel com as tabelas retiradas da Aeronáutica"""
    common = other[other.Station != -1].reset_index(drop=True)
    new = other[other.Station == -1].reset_index(drop=True)
    common = common.loc[:, ['Frequency', 'Description', 'Service', 'Station']]
    df['Description'] = df.Description.astype('string')
    other['Description'] = other.Description.astype('string')
    df = pd.merge(df, common, on=['Frequency', 'Service', 'Station'], how='left')
    df.loc[df.Description_y.notna(), 'Description_x'] = df.loc[df.Description_y.notna(), 'Description_x'] + ' | ' + df.loc[df.Description_y.notna(), 'Description_y']
    df.drop(columns=['Description_y'], inplace=True)
    df.rename(columns={'Description_x': 'Description'}, inplace=True)
    return pd.concat([df, new], ignore_index=True).fillna('').sort_values('Frequency').reset_index(drop=True)
...
def update_base(pasta, up_stel=False, up_radcom=False, up_mosaico=False, up_icao=False):
    ...
    mosaico['Fonte'] = 'MOS'
    rd = pd.concat([mosaico, radcom, stel]).sort_values("Frequência").reset_index(drop=True)
...
def read_icao(pasta, update=False):
    ...
    if update:
        ...
        #update_icao(pasta)
        raise NotImplementedError("Atualizar da base de dados do Frequency Finder e Canalização VOR/ILS/DME não implementado")

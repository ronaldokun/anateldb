...
[![](https://img.shields.io/badge/scraper-gazpacho-C6422C.png)](https://github.com/maxhumber/gazpacho)
...
Os dados no formato atual em excel são disponibilizados na pasta dados
no repositório https://github.com/ronaldokun/anateldb.

Os dados em formato otimizado `.parquet.gzip` são disponibilizados junto
ao repositório na pasta `dados` juntamente com as versões das bases
individuais. Futuramente o objetivo é descontinuar a liberação de
arquivos em formato excel e disponibilizar apenas os dados em formato
otimizado.
...
Aeronáutica, salva e retorna o arquivo
...
A função seguinte baixa os dados diretamente da interface pública online
do [Spectrum E](http://sistemas.anatel.gov.br/se/public/view/b/srd.php)
...
A função <code>update_stel</code> é bem mais lenta que as demais, dado
que o banco de dados do STEL é antigo e abarca todos os registros de
outorga de serviços de telecomunicações da ANATEL, com mais de
**400.000** registros ativos. Esse banco de dados é atualizado 1 vez ao
dia à meia-noite e remete ao estado do dia anterior, portanto não faz
sentido atualizá-lo mais de 1 vez por dia.
...
A função
[update_base](https://ronaldokun.github.io/anateldb\updates.html#update_base)
combina as 3 bases anteriores e uniformiza os campos:
...
Se o argumento <code>update=True</code> for fornecido ou arquivo local
não existir, a base de dados é atualizada por meio da função
<code>update_radcom</code>.
...
**Os dados do Stel não serão ilustrados aqui por se tratar de dados de
telecomunicação privados, os demais dados de radiodifusão são públicos e
disponíveis para qualquer interessado consultar**

Se o argumento <code>update=True</code> for fornecido ou arquivo local
não existir, a base de dados é atualizada por meio da função
<code>update_stel</code>.
...
Se o argumento <code>update=True</code> for fornecido ou o arquivo local
não existir, a base de dados é atualizada por meio da função
<code>update_mosaico</code>.

> A função <code>update_mosaico</code> usa a base de dados Pública do
> Spectrum E, portanto basta somente estar conectado na internet ??.

A função
[update_base](https://ronaldokun.github.io/anateldb\updates.html#update_base)
combina as 3 atualizações de base anteriormente descritas.
